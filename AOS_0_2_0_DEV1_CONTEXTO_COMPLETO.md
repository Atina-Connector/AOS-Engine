...
# Revision cientifica SENS-GLJGL-02

SENS-GLJGL-02 se construye sobre la correccion de paridad y publicacion de
SENS-GLJGL-01. Recupera el ajuste polinomico historico, incluido grado 5, como
una opcion explicita del usuario y no como una operacion oculta.

Modos disponibles:

1. `DISCRETO`: utiliza solo puntos fisicos y no ejecuta `polyfit`.
2. `POLINOMICO_INFORMATIVO`: superpone la curva derivada sin cambiar la
   recomendacion discreta.
3. `POLINOMICO_VERIFICADO`: estima un maximo interior por derivada cero y lo
   recalcula con el evaluador canonico GL/JGL.

El ajuste usa solamente puntos aceptados, no extrapola, no une ramas separadas
y no modifica los datos del solver. El reporte conserva por separado la tabla
fisica, los coeficientes y diagnosticos del polinomio y la verificacion del
optimo. Los nucleos `GL_sim`, balance nodal y solvers JGL permanecen sin cambios.

---

# Revision cientifica SENS-GLJGL-03

SENS-GLJGL-03 se construye de forma acumulativa sobre SENS01 y SENS02. Corrige
la sensibilidad JGL cuando el usuario fuerza `Qiny` y el caso contiene
`P_iny_sup = 0` o una presion no informada. El cero importado se preserva y el
usuario selecciona explicitamente uno de estos modos:

1. `DERIVADA_DESDE_QINY`: calcula la presion minima requerida por Qiny y tobera.
2. `PRESION_DISPONIBLE`: verifica si la presion superficial informada permite cada punto.
3. `SIN_PRESION_MOTRIZ`: confirma cero fisico y rechaza la operacion JGL.

La simulacion puntual JGL con `Qiny` fijo usa la misma seleccion motriz, de modo que el punto individual y el barrido no diverjan por una interpretacion distinta de la presion. La auditoria conserva por separado la presion importada original, la configurada, la disponible, la requerida y la efectiva.

El contrato por punto conserva:
...
La sensibilidad publica la presion de succion, el diferencial motriz requerido,
la presion motriz de fondo, el aporte de la columna de gas, las perdidas del
conducto de inyeccion, la presion superficial minima, el margen y el limite de
`Qiny` por presion. Tambien genera una grafica de presiones absolutas y una
descomposicion de sus componentes.

La presion derivada es un escenario de diseno y no una medicion. Cuando existe
una presion disponible insuficiente, el punto puede conservarse como curva de
diseno en modo derivado, pero no queda habilitado para el optimo. En modo
`PRESION_DISPONIBLE`, un punto no factible es rechazado.

SENS03 mantiene el tratamiento polinomico explicito de SENS02, incluido el
quintico historico. Los puntos del solver siguen siendo la fuente primaria. La
validacion dinamica y el benchmark `MDM-2064` son obligatorios antes de promover
el hotfix.
