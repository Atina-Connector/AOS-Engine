function figs = cgf_plot_resultado(sol,visible)
  ...
  f2=figure('Visible',visible,'Name','CGF - Mapa compresor');c=sol.compresor;s=sol.param.cgf_rpm/c.rpm_base;q=c.Qcorr_Sm3_d*s;pr=exp(log(c.PR_base)*s^2);plot(q,pr,'LineWidth',1.8);hold on;if isfield(sol,'punto'),plot(sol.punto.mapa.Qcorr_Sm3_d,sol.punto.mapa.PR,'ko','MarkerFaceColor','k');endif;plot([c.Q_surge*s c.Q_surge*s],ylim(),'r--');plot([c.Q_choke*s c.Q_choke*s],ylim(),'m--');grid on;xlabel('Caudal corregido (Sm3/d)');ylabel('Relacion de presion');title(sprintf('%s | %.0f rpm',c.modelo,sol.param.cgf_rpm));legend('Mapa','Operacion','Surge','Choke','Location','best');figs(end+1)=f2;
