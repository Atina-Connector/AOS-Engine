function hfig = plot_erosion_taitel(arg1, varargin)
  ...
  subplot(2,3,6);
  axis off;
  txt = resumen_texto(perfil, sistema);
  text(0.02, 0.98, txt, 'Units', 'normalized', 'VerticalAlignment', 'top', 'FontName', 'monospace');
end
...
function txt = resumen_texto(perfil, sistema)
  ...
  txt = sprintf(['Sistema: %s\n' ...
                 ...
                 sistema, length(perfil.MD), min(perfil.MD), max(perfil.MD), ...
                 min(perfil.TVD), max(perfil.TVD), dstr, ql_m3d, ql_bpd, qgf, qgi, qgt, ...
                 uso_erosion, perfil.MD(idxe), margen_carga, perfil.MD(idxc));
end
